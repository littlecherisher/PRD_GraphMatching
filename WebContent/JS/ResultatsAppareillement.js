...
function getNewChemData(){
	// Partie 12：Lisez les données de la table de pages et générez un tableau
	// bidimensionnel data_VC0 (pour la correspondance à double image) et
	// getGraphNum.
	{
		var data_VC0 = [];
	...
	// Partie 13: Lisez le fichier .dat et effectuez une division des données
	// pour générer un tableau tridimensionnel de GraphsData_V_V et
	// Graphs_Data_E_E
	{
		var getGraphesData = document.getElementById('getGraphesData').value;
		var cutGraphesData = getGraphesData.split("V,id,attributes:0,");
		var GraphesData = [];
		var GraphesNewName = [];
		var yGraphesNewName0 = cutGraphesData[0].indexOf(' ');
		if (yGraphesNewName0 < 1) {
		...
		for (var i = 1; i < cutGraphesData.length; i++) {
			var times = (cutGraphesData[i].split('|')).length - 1;
			var locations = find(cutGraphesData[i], '|', times - 1);
			var locations2 = cutGraphesData[i].indexOf(' ');
			if (locations2 < 1) {
				GraphesNewName[i] = cutGraphesData[i].substring(locations + 4);
			} else {
				GraphesNewName[i] = cutGraphesData[i].substring(locations + 4,
						locations2);
			}
			GraphesData[i - 1] = GraphesNewName[i - 1] + cutGraphesData[i];
			var times0 = (GraphesData[i - 1].split('|')).length - 1;
			var locations0 = find(GraphesData[i - 1], '|', times0 - 1);
			GraphesData[i - 1] = GraphesData[i - 1]
					.substring(0, locations0 + 4);
		}
		var GraphesData_Name = [];
		var GraphesData_VE = [];
		var GraphesData_V = [];
		...
		for (var i = 0; i < GraphesData.length; i++) {
			...
			y = GraphesData[i].indexOf('E,from,to');
			GraphesData_Name[i] = GraphesData[i].substring(x, y)
			x = GraphesData[i].indexOf('attributes:0,');
			GraphesData_VE[i] = GraphesData[i].substring(x + 12);
			x = GraphesData_VE[i].indexOf('E,');
			GraphesData_V[i] = GraphesData_VE[i].substring(0, x);
			GraphesData_E[i] = GraphesData_VE[i].substring(x);
			...
			for (j = 0; j < cutGraphesData_E[i].length - 1; j++) {
				...
				GraphesData_E_E[i][j][0] = GraphesData_E_Z[i][j][0];
				GraphesData_E_E[i][j][1] = GraphesData_E_Z[i][j][1];
				GraphesData_E_E[i][j][2] = 1;
			}
		}
	}
	// Partie 14: Requête de données pour générer un tableau bidimensionnel de
	// deux images sélectionnées par l'utilisateur actuel
	// (data_V1,data_V2,data_E1,data_E2)
	{
		var moleculeNo = [];
	...
	// Partie 15：Pré-traitement des données, améliorez la disponibilité des
	// données grâce à la conversion de format, générez des données
	// tridimensionnelles data_V et data_E
	{
		var data_V = [];
	 ...
	 */
	{
		for (var justpre = 0; justpre < 3; justpre++) {
			...
			if (flag == 0) {// Configuration du paramètre du système de gravité
				...
				symbloX = graphpre.offsetWidth * k;
				symbloY = graphpre.offsetHeight / 2;
				repulsion = 20000 / (data_V1.length + data_V2.length);
				edgeLength = 400;
			}
			var data_c = [];
			var link_c = [];
...
function draw(iflag) {
	if (iflag == 0) {
		...
		if (getDat.indexOf('chem:') >= 0) {
			...
			document.getElementById("graph").style.display = "";
			getSimpleData();
		} else if(getDat.indexOf('chem:') < 0 && getDat.indexOf('x:') < 0 && getDat.indexOf('y:') < 0 && getDat.indexOf('V,id') >= 0  && getDat.indexOf('E,from,to') >= 0){
			document.getElementById("graph").style.display = "";
			getNewChemData();
			// alert("PAH/Autres");
		}
